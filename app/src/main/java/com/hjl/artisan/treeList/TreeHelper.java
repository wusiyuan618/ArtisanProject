...
public class TreeHelper {
	...
	public static <T> List<Node> convertData2Nodes(List<T> datas, boolean isHide)
		...
		for (T t : datas) {
			...
			for (Field f : declaredFields) {
				...
				if ("qualifiendProbability".equals(f.getName())) {
					continue;
				}
			...
			if(t instanceof CheckPointTreeBean){
				node.setData(((CheckPointTreeBean) t).getData());
				node.setQualifiendProbability(((CheckPointTreeBean) t).getQualifiendProbability());
			}
	...
	public static void setNodeIcon(Node node) {
		if (node.getChildrenNodes().size() > 0 && node.isExpand()) {
			node.setIcon(R.mipmap.gengduo_close_spotcheck);
		} else if (node.getChildrenNodes().size() > 0 && !node.isExpand()) {
			node.setIcon(R.mipmap.gengduo_spotcheck);
		} else
			node.setIcon(-1);
	}
